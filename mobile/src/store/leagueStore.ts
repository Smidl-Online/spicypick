...
type League = { id: string; tier: string; weekStart: string; weekEnd: string; };
type LeagueState = { league: League | null; userRank: number; userWeeklyXp: number; leaderboard: LeaderboardEntry[]; isLoading: boolean; fetchCurrent: () => Promise<void>; };

export const useLeagueStore = create<LeagueState>((set) => ({
  league: null, userRank: 0, userWeeklyXp: 0, leaderboard: [], isLoading: false,
  fetchCurrent: async () => {
    set({ isLoading: true });
    try {
